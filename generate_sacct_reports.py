...
# Command template
#    sacct -P -r {partition_list} -S YYYY-MM-01 -E YYYY-{MM+1}-01 -o "JobID%20,JobName,User,Account%25,Partition,NodeList%20,Elapsed,State,ExitCode,ReqMem,MaxRSS,MaxVMSize,ReqTRES%60,AllocTRES%60"
...
def main():
    ...
    for d in range(ndates - 1):
        ...
        for p in partitions:
            ...
            sacct_cmd = f'sacct -P -r {p} -S {start_date} -E {end_date} -o JobID%20,JobName,User,Account%25,Partition,NodeList%20,Elapsed,State,ExitCode,ReqMem,MaxRSS,MaxVMSize,ReqTRES%60,AllocTRES%60'
